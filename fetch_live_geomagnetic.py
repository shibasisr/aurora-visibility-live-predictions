...
OMNI2 has a ~2-week publication lag -- unusable for live predictions. NOAA SWPC publishes the same underlying measurements
...
# Standard Kp -> ap conversion table, matching the convention used at training time.
...
def get_live_geomagnetic_indices():
    ...
    can be checked before using this in a prediction.
    """
    ...
    return {
        ...
        # Kp-implied ap (via KP_TO_AP), not a directly-measured ap -- NOAA's real-time feed
        # doesn't publish the 3-hourly linear ap index itself, only NOAA's own Kp estimate
        # (which also isn't identical to the definitive Potsdam/GFZ Kp that OMNI2 uses --
        # can differ by a fraction of a Kp unit).
